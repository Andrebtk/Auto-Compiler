...
int main(int argc, char **argv){
    ...
    printf("\033[1;31m[2]\033[0m - Flask \n\nauto > ");
    
    scanf("%d",&input);
    
    if(input != 0x01 && input != 0x02){ printf("\n\033[1;31m[!]\033[0m Please select one fo the options \n"); return -1; }
...
void dif(int z){
    ...
    if (z==0x02){ printf("\033[1;31mStill under development\n\033[0m"); exit(1); }
    
    printf("Input the name of the file : ");

    scanf("%s", x);
    
    if (z==0x01){ snprintf(command, sizeof(command), "gcc %s -o auto && ./auto", x); }
    if (z==0x02){ snprintf(command, sizeof(command), "flask run &", x); }
